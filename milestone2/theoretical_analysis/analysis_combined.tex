...
% ============================================================================
% PREAMBLE (Embedded - No external .sty file needed)
% ============================================================================
...
\pagestyle{fancy}
\fancyhf{}
\rhead{Lamaan Ali Bukhsh}
\lhead{Milestone 2: Complexity Analysis}
\cfoot{\thepage}
...
\newcommand{\note}[1]{\textit{\textcolor{darkgreen}{[Note: #1]}}}
...
This indicates that with perfect load balancing and zero communication, an ideal 4-processor PRAM can achieve $\sim 4.0x$ speedup (limited by $P=4$ available processors).
...
\subsubsection{MPI\_Scatterv (Master to Workers)}
...
\subsubsection{MPI\_Gatherv (Workers to Master)}
...
\textbf{Interpretation}: Communication overhead is $\sim 20x$ the theoretical computation time. This is the primary bottleneck in our distributed system.
...
\textbf{Interpretation}: With our estimated 26.4\% sequential fraction, a 4-node cluster achieves a predicted speedup of $\sim 2.23x$ compared to single-node execution.
...
Even with infinite processors, our system is limited to $\sim 3.79x$ speedup due to the 26.4\% sequential fraction.
...
This theoretical analysis predicts $S(4) = 2.23x$ speedup on our 4-node cluster. Sharjeel Chandna will validate these predictions empirically by:
...
\begin{enumerate}
    ...
    \item \textbf{Amdahl validation}: Compare measured $S(P)$ to predictions. If $S(4) \approx 2.0-2.5$, theory is validated.
\end{enumerate}
...
\begin{enumerate}
    \item \textbf{Inherent parallelism}: The 2D FFT has $\sim 14x$ parallelism ratio (Work/Span), allowing good speedup on multiple processors.
    
    \item \textbf{Sequential bottleneck}: Global matrix transposition and MPI communication comprise $\sim 26.4\%$ of total execution time, limiting speedup to $S(4) = 2.23x$.
    
    \item \textbf{Asymptotic limit}: Even with infinite processors, speedup is capped at $3.79x$ (from Amdahl's Law), reflecting the 26.4\% sequential fraction.
